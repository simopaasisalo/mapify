...
export class SymbolMenu extends React.Component<ISymbolMenuProps, ISymbolMenuStates>{
      ...
      let headers = this.props.layer.headers.filter(function(head) { return head.type === 'number' });
        let state: ISymbolMenuStates = {
          selectableHeaders:headers,
            ...
            icons: prev.icons ? prev.icons : [{ iconFA: 'fa-anchor', iconShape: 'circle' }],
    ...
    faIconChanged(iconFA) {
        ...
        icons[this.state.currentIconIndex].iconFA = iconFA;
        this.setState({
            icons: icons
        });
    }
    iconShapeChanged(shape: 'circle' | 'square' | 'star' | 'penta') {
        let icons = this.state.icons;
        icons[this.state.currentIconIndex].iconShape = shape;
        this.setState({
            icons: icons
        });
    }
    ...
    getIconShape(shape: string) {
        let circleSVG =
            <svg viewBox="0 0 69.529271 95.44922" height="40" width="40">
                <g transform="translate(-139.52 -173.21)">
                    <path fill='transparent' stroke="#999999" d="m174.28 173.21c-19.199 0.00035-34.764 15.355-34.764 34.297 0.007 6.7035 1.5591 12.813 5.7461 18.854l0.0234 0.0371 28.979 42.262 28.754-42.107c3.1982-5.8558 5.9163-11.544 6.0275-19.045-0.0001-18.942-15.565-34.298-34.766-34.297z"/>
                </g>
        ...
        let squareSVG =
            <svg viewBox="0 0 69.457038 96.523441" height="40" width="40">
                <g transform="translate(-545.27 -658.39)">
                    <path fill='transparent' stroke="#999999" d="m545.27 658.39v65.301h22.248l12.48 31.223 12.676-31.223h22.053v-65.301h-69.457z"/>
                </g>
            </svg>
        let starSVG =
            <svg height="40" width="40" viewBox="0 0 77.690999 101.4702"><g transform="translate(-101.15 -162.97)">
                <g transform="matrix(1 0 0 1.0165 -65.712 -150.28)">
                    <path  fill='transparent' stroke="#999999" d="m205.97 308.16-11.561 11.561h-16.346v16.346l-11.197 11.197 11.197 11.197v15.83h15.744l11.615 33.693 11.467-33.568 0.125-0.125h16.346v-16.346l11.197-11.197-11.197-11.197v-15.83h-15.83l-11.561-11.561z"/></g>
            </g>
            </svg>
        let pentaSVG =
            <svg viewBox="0 0 71.550368 96.362438" height="40" width="40">
                <g fill='transparent' transform="translate(-367.08 -289.9)">
                    <path stroke="#999999" d="m367.08 322.5 17.236-32.604h36.151l18.164 32.25-35.665 64.112z"/></g>
            </svg>
        switch (shape) {
            case ('circle'):
                return circleSVG;
    ...
    calculateValues(fieldName: string,steps:number) {
        ...
        for (let i = min; i < max; i += (max - min) / steps) {
            limits.push(i);
            icons.push(this.state.icons[index]? this.state.icons[index]: {iconFA:'fa-anchor',iconShape:'circle'})
            index++;
        }
                                ...
                                {this.state.useIconSteps ?
                                  ...
                                  <Select
                                    ...
                                    <div>
                                        Set icon
                                        <div
                                            onClick={this.toggleIconPick.bind(this, 0) }
                                            style={{
                                                cursor: 'pointer',
                                            ...
                                            {this.getIconShape(this.state.icons[0].iconShape) }
                                            <i style={{ position: 'relative', bottom: 20, right: 28, width:16,height:16 }} className={'fa ' + this.state.icons[0].iconFA}/>
...
{this.state.iconSelectOpen?<div>
                        ...
                        <input type="text" onChange={this.faIconChanged.bind(this) } value={this.state.icons[this.state.currentIconIndex].iconFA}/>
            ...
            for (let i of steps) {
                rows.push(
                    ...
                    <input
                            ...
                            step='any'/>
                            <div
                                onClick={this.toggleIconPick.bind(this,row) }
                                style={{
                                    cursor: 'pointer',
                                    display: 'inline-block',
                                    textAlign:'center',
                                ...
                                >
                                {this.getIconShape(this.state.icons[row].iconShape) }
                                <i style={{ position: 'relative', bottom: 20, right: 28, width:16,height:16 }} className={'fa ' + this.state.icons[row].iconFA}/>
                            </div>
                    </li>);
                row++;
            }
            return <div>
            Set the <i>lower limit</i> and icon
            <br/>
